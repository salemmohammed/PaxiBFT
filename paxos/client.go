...
func (c *Client) Get(key PaxiBFT.Key) (PaxiBFT.Value, error) {
	c.HTTPClient.CID++
	switch *read {
	case "leader":
...
func (c *Client) readLeader(key PaxiBFT.Key) (PaxiBFT.Value, error) {
	if c.ballot == 0 {
		v, meta, err := c.HTTPClient.RESTGet(c.ID, key)
		c.ballot = PaxiBFT.NewBallotFromString(meta[HTTPHeaderBallot])
		return v, err
	}
	// check ballot number
...
func (c *Client) readQuorum(key PaxiBFT.Key) (PaxiBFT.Value, error) {
	majority := c.N/2 + 1
	barrier := -1
	numReachedBarrier := 0
	numInProgress := 0
	var value PaxiBFT.Value
...
func (c *Client) readAny(key PaxiBFT.Key) (PaxiBFT.Value, error) {
